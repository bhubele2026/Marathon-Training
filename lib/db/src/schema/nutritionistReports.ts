...
import type { AlcoholStats } from "./alcoholEntries";
...
export type InsightId =
  ...
  // week, `dryDays` = the positive metric, dry days vs the weekly target.
  | "alcohol"
  | "dryDays";
...
export type NutritionInsight = {
  ...
  // --- alcohol tiles only (id 'alcohol' | 'dryDays') ---
  // The full deterministic alcohol read rides on both alcohol insights so the
  // fun tiles (7-day strip, week-over-week dry days, impact drawer) draw from
  // one source. Undefined on every non-alcohol insight.
