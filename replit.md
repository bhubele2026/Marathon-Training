...
- **Real strength-workout model (Phase 1).** `AiDay` (lib/plan-knowledge/src/types.ts) now carries an ordered `strengthBlocks` array — each block is a real movement: `movement`, `pattern` (squat / hinge / horizontal_push / horizontal_pull / vertical_push / vertical_pull / lunge / carry / core), `sets`, plus optional `tempo` / `restSec` / per-movement `equipment` / `tonalMode` / `cue`. **Reps and weight are intentionally NOT prescribed** — on Tonal the program + its digital auto-weight drive reps and load, so `reps` / `loadType` / `loadValue` are optional and normally left empty; Studio schedules the movements, sets, emphasis and Tonal mode, and progresses week-to-week structurally (movements / volume / emphasis), letting Tonal's auto-weight own the load progression. Week-over-week change in these blocks IS the progression (week 1 ≠ week 8). The plan is recomp-first and goal-agnostic: `goalKind` (default `"recomp"`) replaced a required `raceKind`; `raceKind`/pace/distance are optional and only meaningful on run plans; `startDate` no longer has to be a "campaign" Monday — `materializeAiPlan` snaps it to the Monday on/before via `mondayOnOrBefore`, and the `start_not_monday` guardrail is gone. Persisted on `plan_days.strength_blocks` + `workouts.strength_blocks` (jsonb, with seed mirrors), surfaced through the `PlanDay` API (`StrengthBlock` schema), and rendered on Today + the week day cards by the `StrengthBlocks` component (falls back to prose + minutes when null).
...
## Smarter tracking (per-day macros, progress picture, diagnosis, sass)

- **Per-day macro scaling (`nutrition-engine.ts`).** The day target is now an explicit macro split that scales with the day's **training load, never burned calories**: **protein** is anchored to the recomp floor every day with only a small load-scaled bump on the heaviest days (`proteinBumpForLoad`, ≤20 g, never reduced below the floor — no wild swings); **carbs** carry ~all of the clamped (±~120 g) calorie swing — the day-to-day lever; **fat** is the balancer (`balanceFatG`) filling the remaining calories so the three macros sum to the day's target. The AI prompt teaches the same model and the coach's daily line references it (`getDayTarget` feeds `gatherDay`). The day-target cache fingerprint is `v2` so old rows recompute. Heavy day → more carbs + a touch more protein; rest day → carbs ease back.
- **Progress picture (`dashboard-tracking.ts` + route + component).** A selectable window (4/8/12 wk) with: actual **weight vs the per-week target-weight curve** (reuses `targetWeightForWeek` / `weeklyWeightStatus`, with on-track/ahead/behind), **recomp signals** (total-inches `totalInches` trend + strength score — flat scale + inches down + strength up reads as winning), and **adherence** (calorie-hit `calorieHitRate`, protein-hit, session-consistency). recharts in the de-boxed single-accent style.
- **Diagnosis engine (`progress-diagnosis.ts` + `routes/progress.ts`).** `diagnose()` correlates a trailing window (weight trend vs goal rate, calorie + protein adherence, consistency, inches) into RANKED findings, each a real cause + a safe fix. **Decision framework, by rank:** under-floor / losing-too-fast → **supportive** (raise calories; health flag, never a roast); flat scale + inches down → **recomp working** (positive, stay the course); flat + over-target / low-protein / missed-sessions → **sassy** (adherence); adherent-but-flat for weeks → **plateau** (diet break / recompute); else on-track / insufficient-data. **Safety is hard:** a finding can never push below the safe floor or above the safe rate. `GET /api/progress/diagnosis` narrates the top findings in persona (supportive tone enforced for health flags) with a deterministic fallback, persists the latest in the **`progress_diagnosis`** singleton (input-hash cached). Surfaced as the on-demand **"What's going on"** dashboard panel; the weekly recap folds in the top finding.
- **Sass dial (Phase 4).** `persona.ts` turned up — sharper, more theatrical British tough-love aimed at effort/excuses, grudging praise; `session-verdict.ts` copy punched up to match. One voice across the daily line, verdict, weekly recap, and diagnosis. **The wellbeing rails are unchanged** — never the body/worth, always flips supportive on under-eating / too-fast / distress, never below floor or above safe rate. (The optional per-user sass-intensity setting was intentionally NOT built: it would require an `openapi.yaml` change + codegen on the preferences slice — not cheap.)

> **DB push (smarter tracking):** push the new **`progress_diagnosis`** table (`pnpm --filter @workspace/db run push`). No new deps; the new endpoints (`/api/dashboard/tracking`, `/api/progress/diagnosis`) are hand-fetched (not in `openapi.yaml`), so no codegen. Diagnosis narration needs `ANTHROPIC_API_KEY`; without it the deterministic finding text is shown.

## Always-on coach + premium UI

- **Coach is everywhere.** A slim, on-brand, **dismissible** coach strip (`components/coach-dock.tsx`) lives in the shell above page content and reacts to the CURRENT screen + today's data: nutrition → macros, plan → adherence, body → recomp, dashboard → overall read, today → the session. Fed by `GET /api/coach/line?context=…` (hand-fetched; reuses `gatherDay` + `buildDataSummary` + the shared persona; deduped in-process by context+input-hash so the model isn't re-called until the day changes). Non-blocking, fails to nothing (AI unconfigured / empty day / dismissed-for-the-day via localStorage). The Today in-page daily line was removed (the dock covers it); the per-session verdict strip stays.
- **Max sass, rails intact.** `lib/plan-knowledge/persona.ts` is cranked to maximum theatrical British tough-love — one voice across the dock, daily line, verdicts, weekly summary, and diagnosis. The wellbeing rails are unchanged and OVERRIDE the intensity: meanness at effort/excuses never the body/worth; never below the safe floor or above the safe rate; **full supportive flip** on under-eating / too-fast loss / distress; dismissible. (A per-user sass-level setting was intentionally skipped — it would need an `openapi.yaml` change + codegen on the preferences slice.)
- **BH Studio orange identity.** Single accent = orange (`--primary` hue ~22/24) in both light and dark; charts are orange + a neutral gray ramp (phase/program coloring is the only semantic multi-hue, kept on purpose). The old "teal" comments in `index.css` + `visual-themes` were stale drift and are corrected to orange — do NOT reintroduce teal.
- **Premium visual system** (`index.css`, central): a deep near-black dark canvas with cards lifted a clear step for layered depth (orange reads premium on dark); one soft layered shadow system (`shadow-card` / `shadow-card-lg`, dialed per theme); tabular figures app-wide (`font-feature-settings: tnum`); smooth restrained transitions on every interactive element; refined custom scrollbars. Opt-in hero accents used sparingly: `.glow-primary` (orange glow on the single hero number — applied to the Eat-today calories), `.ring-glow-primary`, and `.gradient-primary` (restrained orange gradient on the primary Log CTA). Square corners (`--radius: 0`) remain the identity. Restrained framer-motion route transitions (~180ms) in `layout.tsx`.
